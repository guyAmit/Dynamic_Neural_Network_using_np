...
if __name__ == '__main__':
    ...
    net.add_layer(units=40, activation=net_utills.relu, dropout=0.8)
    net.add_layer(units=40, activation=net_utills.relu)
    net.add_layer(units=20, activation=net_utills.relu, dropout=1)
    net.add_layer(units=20, activation=net_utills.relu, l2_regulaizer=0, dropout=0.8)
    net.add_layer(units=5, activation=net_utills.softmax, l2_regulaizer=0.7)
    ...
    costs_train, succ_train, costs_val, succ_val = \
        net.fit(X_train=X_train, C_train=Y_train, X_val=X_val, C_val=Y_val, epoch=50, batch_size=64, p=True)
